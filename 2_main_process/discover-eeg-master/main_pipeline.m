...
%
% This script processes EEG data in three main stages:
% 1. Import raw BIDS-formatted EEG data
% 2. Preprocess the data (artifact removal, segmentation, etc.)
% 3. Extract brain features (power, connectivity, graph measures)
%
% NOTE: To process both control and sport dependence groups, you need to:
% 1. Run this pipeline once with RawDataPath pointing to control_close_bids
% 2. Update the params.json file to point to sport_dependence_close_bids
% 3. Run this pipeline again for the sport dependence group
% 4. For group comparisons, you'll need to analyze the outputs separately

clear all; close all;
rng('default'); % For reproducibility - See discussion in https://sccn.ucsd.edu/pipermail/eeglablist/2022/016932.html

% Define the parameters from params.json
% NOTE: Check that RawDataPath in params.json points to the correct BIDS directory:
params = define_params('params_sd_weighted.json');
...
if exist(fullfile(params.PreprocessedDataPath,[params.StudyName '.study']),'file')
    ...
    % NOTE: This will import data from the directory specified in params.RawDataPath
...
if params.BrainFeatExtr
    ...
    for iRec=1:length(STUDY.datasetinfo)
        ...
        for iFreq = 1:length(freqs)
            ...
            % Check if we should use weighted network analysis
            useWeighted = false;
            if isfield(params, 'UseWeightedGraphs')
                useWeighted = params.UseWeightedGraphs;
            end
            
            if useWeighted
                % Compute graph measures for weighted network
                if ~exist(fullfile(params.GraphPath,[bidsID '_graph_dwpli_' freqs{iFreq} '_weighted.mat']),'file')
                    try
                        fprintf('Computing weighted graph measures for %s, %s, dwpli...\n', bidsID, freqs{iFreq});
                        compute_graph_measures_weighted(params,bidsID,freqs{iFreq},'dwpli',true);
                        fprintf('Successfully computed weighted graph measures for %s, %s, dwpli.\n', bidsID, freqs{iFreq});
                    catch ME
                        warning([bidsID ' - dwpli - ' freqs{iFreq} ' - Error: ' ME.message]);
                        fprintf('Stack trace:\n');
                    ...
                    fprintf('Weighted graph measures file already exists for %s, %s, dwpli. Skipping.\n', bidsID, freqs{iFreq});
                end
                if ~exist(fullfile(params.GraphPath,[bidsID '_graph_aec_' freqs{iFreq} '_weighted.mat']),'file')
                    try
                        fprintf('Computing weighted graph measures for %s, %s, aec...\n', bidsID, freqs{iFreq});
                        compute_graph_measures_weighted(params,bidsID,freqs{iFreq},'aec',true);
                        fprintf('Successfully computed weighted graph measures for %s, %s, aec.\n', bidsID, freqs{iFreq});
                    catch ME
                        warning([bidsID ' - aec - ' freqs{iFreq} ' - Error: ' ME.message]);
                        fprintf('Stack trace:\n');
                        for k = 1:length(ME.stack)
                            fprintf('  File: %s, Line: %d, Function: %s\n', ME.stack(k).file, ME.stack(k).line, ME.stack(k).name);
                        end
                    ...
                    fprintf('Weighted graph measures file already exists for %s, %s, aec. Skipping.\n', bidsID, freqs{iFreq});
                end
            else
                % Compute graph measures for binary network (default behavior)
        ...
        end  % End of for iFreq = 1:length(freqs) loop
        ...
        for iConMeas = 1:length(conMeas)
            ...
            if useWeighted
                if ~exist(fullfile(params.GraphPath,[bidsID '_' conMeas{iConMeas} '_degree_weighted.svg']),'file') ||...
                        ~exist(fullfile(params.GraphPath,[bidsID '_' conMeas{iConMeas} '_cc_weighted.svg']),'file') ||...
                        ~exist(fullfile(params.GraphPath,[bidsID '_' conMeas{iConMeas} '_global_weighted.svg']),'file')
                    try
                        [f_degree, f_cc, f_global] = plot_graph_measures(params,bidsID,conMeas{iConMeas});
                        % Add drawnow and pause to avoid ColorBar update issues
                        drawnow('update');
                        pause(0.1);
                        saveas(f_degree,fullfile(params.GraphPath,[bidsID '_' conMeas{iConMeas} '_degree_weighted.svg']));
                        close(f_degree);
                        drawnow('update');
                        pause(0.1);
                        saveas(f_cc,fullfile(params.GraphPath,[bidsID '_' conMeas{iConMeas} '_cc_weighted.svg']));
                        close(f_cc);
                        drawnow('update');
                        pause(0.1);
                        saveas(f_global,fullfile(params.GraphPath,[bidsID '_' conMeas{iConMeas} '_global_weighted.svg']));
                        close(f_global);
                    catch ME
                        warning([bidsID ' - ' ME.message]);
                    end
        ...
        end  % End of for iConMeas = 1:length(conMeas) loop
    ...
    end  % End of for iRec=1:length(STUDY.datasetinfo) loop
end  % End of if params.BrainFeatExtr
